...
def do_generate(question: str, style: str, word_limit: int, mode: str,
    ...
    _last_chunks = chunks
    chunks_text  = "\n\n" + ("─" * 60) + "\n\n".join(chunks)
    return answer, translation, chunks_text, backend_status()
...
def do_export(p0, p1, p2, p3, p4, prompt_text):
    """
    Exportiert Bilder + Prompt in workflow/ und ComfyUiVid/input.
    Bilder: a_XXXX_.png, b_XXXX_.png, ...
    Prompt: video.txt (oder novideo.txt wenn keine Bilder).
    ...
    if exported > 0:
        return f"✅ Exportiert: {exported} Bild(er) + {txt_name} → workflow & ComfyUiVid\\input"
    else:
        return f"⚠️ Keine Bilder gefunden — {txt_name} gespeichert"
...
def do_controlnet(p0, p1, p2, p3, p4, prov):
    """
    Generiert ControlNet Captions (Pose + Atmosphere) aus 5 Bildern.
    """
    images = [p0, p1, p2, p3, p4]
    if not any(images):
    ...
    with gr.Blocks(title="RAG — Генератор сценариев") as demo:
        ...
        with gr.Column(visible=False) as timeline_column:
            with gr.Accordion("🎥 Кинематографические приёмы (LTX Timeline)", open=False):
                ...
                with gr.Row():
                    gr.Markdown("&nbsp;")
                    tl_col_1 = gr.Markdown("**Ч1: Начало**")
                    tl_col_2 = gr.Markdown("**Ч2: Развитие**")
                    tl_col_3 = gr.Markdown("**Ч3: Кульминация 1**")
                    tl_col_4 = gr.Markdown("**Ч4: Кульминация 2**")
                    tl_col_5 = gr.Markdown("**Ч5: Финал**")
        ...
        with gr.Accordion("🖼️ Мульти-кадровый таймлайн (T2I / I2V для LTX 2.3)", open=False):
            ...
            with gr.Column(visible=True) as video_col:
                gr.Markdown(
                    "Загрузи от 1 до 5 ключевых кадров. Пропущенные будут заполнены автоматически: "
                ...
                with gr.Row():
                    img_slots = [
                        gr.Image(
                            label   = f"🎥 {['0% (Старт)', '25%', '50%', '75%', '100% (Финал)'][i]}",
                            type    = "filepath",
                            height  = 180,
                            visible = True,
                        )
            ...
            with gr.Column(visible=False) as controlnet_col:
                ...
                with gr.Row():
                    cn_img_slots = [
                        gr.Image(
                            label   = f"🎥 {['0% (Start)', '25%', '50%', '75%', '100% (Final)'][i]}",
                            type    = "filepath",
                            height  = 180,
                            visible = True,
                        )
        ...
        gen_btn.click(
            ...
            fn      = lambda answer, translation: (
                gr.update(visible=bool(translation.strip())),
            ),
            inputs  = [answer_box, answer_ru_box],
            outputs = [answer_translation_col],
        )
        ...
        tl_col_refs = [tl_col_1, tl_col_2, tl_col_3, tl_col_4, tl_col_5]
        ...
        def do_timeline(p0, p1, p2, p3, p4, prov):
